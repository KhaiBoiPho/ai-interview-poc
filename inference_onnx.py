...
def datagen_onnx(whisper_chunks, vae_encode_latents, batch_size=8, delay_frame=0):
    """Data generator for ONNX inference"""
...
def main(args):
    print("="*60)
    print("  MUSETALK ONNX FP16 INFERENCE")
    print("="*60)
    ...
    # Load ONNX models
    print("\nLoading ONNX models...")
    vae = ONNXVAE(model_dir=args.onnx_dir, use_gpu=args.use_gpu)
    unet = ONNXUNet(model_dir=args.onnx_dir, use_gpu=args.use_gpu)

    # Load faster-whisper
    print("\nLoading faster-whisper...")
    whisper_processor = FasterWhisperProcessor(
        model_size=args.whisper_model_size,
        device="cuda" if args.use_gpu else "cpu",
        compute_type="float16" if args.use_gpu else "int8"
    )
    ...
    print(f"\nLoaded inference config: {inference_config}")
    ...
    for task_id in inference_config:
        try:
            ...
            bbox_shift = 0 if args.version == "v15" else inference_config[task_id].get("bbox_shift", args.bbox_shift)
            ...
            # output_vid_name_concat = os.path.join(temp_dir, output_basename + "_concat.mp4")  # noqa: F841
            ...
            print("\nExtracting audio features with faster-whisper...")
            whisper_chunks = whisper_processor.get_whisper_chunk(
                audio_path,
                fps=fps,
            ...
            input_latent_list = []
            for bbox, frame in tqdm(zip(coord_list, frame_list), total=len(frame_list)):
            ...
            for i, (whisper_batch, latent_batch) in enumerate(tqdm(gen, total=total)):
                # UNet forward
                pred_latents = unet.forward(latent_batch, timesteps, whisper_batch)
            ...
            if args.skip_padding:
                ...
                print("Saving frames (quality mode)...")
                for i, res_frame in enumerate(tqdm(res_frame_list)):
                    bbox = coord_list_cycle[i % len(coord_list_cycle)]
                    ori_frame = copy.deepcopy(frame_list_cycle[i % len(frame_list_cycle)])
                    x1, y1, x2, y2 = bbox
...
if __name__ == "__main__":
    ...
    parser.add_argument("--whisper_model_size", type=str, default="base", choices=["tiny", "base", "small", "medium", "large-v2", "large-v3"])
